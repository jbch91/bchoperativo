...
export class AdminService {
  ...
  async getMyClient(): Promise<ClientDto | null> {
    try {
      return await firstValueFrom(this.http.get<ClientDto>(`${this.apiBase}/clients/me`));
    } catch {
      return null;
    }
  }
  ...
  async listUserModules(userId: string): Promise<ModuleDto[]> {
    return firstValueFrom(this.http.get<ModuleDto[]>(`${this.apiBase}/admin/users/${userId}/modules`));
  }

  async updateUserModules(userId: string, modules: string[]): Promise<void> {
    await firstValueFrom(this.http.put(`${this.apiBase}/admin/users/${userId}/modules`, { modules }));
  }
