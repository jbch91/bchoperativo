...
import { createSale, deleteSale, listSaleLines, listSales, updateSale } from './sales.js';
import { createRemision, createRemisionClient, deleteRemision, listRemisionClients, listRemisionLines, listRemisiones, updateRemision } from './remisiones.js';
import { createCashTransaction, deleteCashTransaction, getCashSummary, getCashTransactionById, listCashTransactions, updateCashTransaction } from './cash.js';
...
app.get(
  '/inventory/:clientId/products',
  requireAuth,
  requireAnyPermission(['inventory:view', 'inventory:manage', 'read:all']),
  async (req, res) => {
    const { clientId } = req.params;
    if (req.user.clientId && req.user.clientId !== clientId) {
      return res.status(403).json({ message: 'Sin acceso al cliente.' });
    }
    const rows = await listInventoryProducts(clientId);
...
app.post(
  '/inventory/:clientId/products',
  requireAuth,
  requireAnyPermission(['inventory:create', 'inventory:manage', 'read:all']),
  async (req, res) => {
    const { clientId } = req.params;
    if (req.user.clientId && req.user.clientId !== clientId) {
      return res.status(403).json({ message: 'Sin acceso al cliente.' });
    }
    const { code, articulo, presentacion, marca } = req.body || {};

    if (!code || !articulo || !presentacion || !marca) {
      return res.status(400).json({ message: 'Datos incompletos.' });
    }

    const created = await createInventoryProduct(clientId, {
      code,
      articulo,
      presentacion,
      marca
...
app.patch(
  '/inventory/:clientId/products/:id',
  requireAuth,
  requireAnyPermission(['inventory:edit', 'inventory:manage', 'read:all']),
  async (req, res) => {
    ...
    await updateInventoryProduct(clientId, id, { code, articulo, presentacion, marca });
    return res.json({ ok: true });
  }
);

app.post(
  '/inventory/:clientId/products/bulk',
  requireAuth,
  requireAnyPermission(['inventory:create', 'inventory:manage', 'read:all']),
  async (req, res) => {
    ...
    const result = await createInventoryProductsBulk(clientId, normalized);
    return res.json(result);
  }
);

app.get(
  '/inventory/:clientId/summary',
  requireAuth,
  requireAnyPermission(['inventory:view', 'inventory:manage', 'read:all']),
  async (req, res) => {
    const { clientId } = req.params;
    ...
    const rows = await listInventorySummary(clientId);
    return res.json(rows);
  }
);

app.get(
  '/inventory/:clientId/entries',
  requireAuth,
  requireAnyPermission(['inventory:view', 'inventory:manage', 'read:all']),
  async (req, res) => {
    const { clientId } = req.params;
    ...
    const rows = await listInventoryEntries(clientId);
    return res.json(rows);
  }
);
...
app.post(
  ...
  async (req, res) => {
    ...
    const { productId, costo, ivaTipo, costoBase, ivaValor, costoTotal, ventaFactor, precioVenta, fechaVencimiento, lote, invima, cantidad } = req.body || {};
    if (!productId || !fechaVencimiento || !lote || !invima) {
      return res.status(400).json({ message: 'Datos incompletos.' });
    }
    ...
    const created = await createInventoryEntry(productId, {
...
app.delete(
  '/inventory/:clientId/products/:id',
  requireAuth,
  requireAnyPermission(['inventory:delete', 'inventory:manage', 'read:all']),
  async (req, res) => {
    const { clientId, id } = req.params;
    if (req.user.clientId && req.user.clientId !== clientId) {
      return res.status(403).json({ message: 'Sin acceso al cliente.' });
    }
    await deleteInventoryProduct(clientId, id);
    return res.json({ ok: true });
  }
);

app.delete(
  '/inventory/:clientId/entries/:id',
  requireAuth,
  requireAnyPermission(['inventory:delete', 'inventory:manage', 'read:all']),
  async (req, res) => {
    const { clientId, id } = req.params;
    ...
    await deleteInventoryEntry(clientId, id);
    return res.json({ ok: true });
  }
);
...
app.patch(
  ...
  async (req, res) => {
    ...
    const { costo, ivaTipo, costoBase, ivaValor, costoTotal, ventaFactor, precioVenta, fechaVencimiento, lote, invima, cantidad } = req.body || {};
    if (!fechaVencimiento || !lote || !invima) {
      return res.status(400).json({ message: 'Datos incompletos.' });
    }
    const normalizedIva = ['gravado','excluido','exento'].includes(String(ivaTipo || '').toLowerCase())
      ? String(ivaTipo).toLowerCase()
...
app.post(
  '/sales/:clientId',
  requireAuth,
  requireAnyPermission(['sales:create', 'inventory:manage', 'read:all']),
  async (req, res) => {
    const { clientId } = req.params;
    if (req.user.clientId && req.user.clientId !== clientId) {
      return res.status(403).json({ message: 'Sin acceso al cliente.' });
    }
    const { totalBase, totalIva, total, lines, buyerName, buyerDocument, buyerAddress, consumptionArea, consumptionNote, saleType, serviceType, paymentMethod } = req.body || {};
    if (!Array.isArray(lines) || lines.length === 0) {
      return res.status(400).json({ message: 'Sin líneas de venta.' });
    }
      const sale = await createSale(clientId, {
        totalBase: Number(totalBase) || 0,
        totalIva: Number(totalIva) || 0,
        total: Number(total) || 0,
        lines,
        buyerName: String(buyerName || '').trim(),
        buyerDocument: String(buyerDocument || '').trim(),
        buyerAddress: String(buyerAddress || '').trim(),
        consumptionArea: String(consumptionArea || '').trim(),
        consumptionNote: String(consumptionNote || '').trim(),
        saleType: String(saleType || 'producto').trim(),
        serviceType: String(serviceType || '').trim(),
        paymentMethod: String(paymentMethod || 'efectivo').trim(),
...
app.get(
  '/sales/:clientId',
  requireAuth,
  requireAnyPermission(['sales:view', 'inventory:manage', 'read:all']),
  async (req, res) => {
    const { clientId } = req.params;
    if (req.user.clientId && req.user.clientId !== clientId) {
      return res.status(403).json({ message: 'Sin acceso al cliente.' });
    }
    const rows = await listSales(clientId);
    return res.json(rows);
  }
);

app.get(
  '/sales/:clientId/:saleId/lines',
  requireAuth,
  requireAnyPermission(['sales:view', 'inventory:manage', 'read:all']),
  async (req, res) => {
    ...
    const rows = await listSaleLines(saleId);
    return res.json(rows);
  }
);

app.get(
  '/cash/:clientId/summary',
  requireAuth,
  requireAnyPermission(['cash:view', 'cash:manage', 'read:all']),
  async (req, res) => {
    const { clientId } = req.params;
    if (req.user.clientId && req.user.clientId !== clientId) {
      return res.status(403).json({ message: 'Sin acceso al cliente.' });
    }
    const summary = await getCashSummary(clientId);
    return res.json(summary);
  }
);

app.get(
  '/cash/:clientId/transactions',
  requireAuth,
  requireAnyPermission(['cash:view', 'cash:manage', 'read:all']),
  async (req, res) => {
    const { clientId } = req.params;
    ...
    const rows = await listCashTransactions(clientId, {
      type: req.query.type,
...
app.post(
  '/cash/:clientId/transactions',
  requireAuth,
  requireAnyPermission(['cash:create', 'cash:manage', 'read:all']),
  async (req, res) => {
    const { clientId } = req.params;
    if (req.user.clientId && req.user.clientId !== clientId) {
      return res.status(403).json({ message: 'Sin acceso al cliente.' });
    }
    const { type, category, amount, description, source } = req.body || {};
    if (!type || !category || !amount) {
      return res.status(400).json({ message: 'Datos incompletos.' });
    }
    if (source === 'opening' && !req.user.roles?.includes('superuser')) {
      return res.status(403).json({ message: 'Solo superuser puede abrir caja.' });
    }
    const created = await createCashTransaction(clientId, {
      type,
      category,
...
app.put(
  '/cash/:clientId/transactions/:id',
  requireAuth,
  requireAnyPermission(['cash:edit', 'cash:manage', 'read:all']),
  async (req, res) => {
    const { clientId, id } = req.params;
    if (req.user.clientId && req.user.clientId !== clientId) {
      return res.status(403).json({ message: 'Sin acceso al cliente.' });
    ...
    const existing = await getCashTransactionById(clientId, id);
    if (!existing) {
      return res.status(404).json({ message: 'Movimiento no encontrado.' });
    }
    if (existing.source === 'opening' && !req.user.roles?.includes('superuser')) {
      return res.status(403).json({ message: 'Solo superuser puede editar saldo inicial.' });
    }
    await updateCashTransaction(clientId, id, {
      type,
      category,
      amount: Number(amount) || 0,
      description: String(description || '').trim()
...
app.delete(
  '/cash/:clientId/transactions/:id',
  requireAuth,
  requireAnyPermission(['cash:delete', 'cash:manage', 'read:all']),
  async (req, res) => {
    const { clientId, id } = req.params;
    if (req.user.clientId && req.user.clientId !== clientId) {
      return res.status(403).json({ message: 'Sin acceso al cliente.' });
    ...
    if (existing.source === 'opening' && !req.user.roles?.includes('superuser')) {
      return res.status(403).json({ message: 'Solo superuser puede eliminar saldo inicial.' });
    }
    await deleteCashTransaction(clientId, id);
    return res.json({ ok: true });
  }
);

app.put(
  '/sales/:clientId/:saleId',
  requireAuth,
  requireAnyPermission(['sales:edit', 'inventory:manage', 'read:all']),
  async (req, res) => {
    const { clientId, saleId } = req.params;
    if (req.user.clientId && req.user.clientId !== clientId) {
      return res.status(403).json({ message: 'Sin acceso al cliente.' });
    ...
    try {
      const result = await updateSale(clientId, saleId, {
        totalBase: Number(totalBase) || 0,
        totalIva: Number(totalIva) || 0,
        total: Number(total) || 0,
        buyerName: String(buyerName || '').trim(),
      ...
      return res.status(400).json({ message: error?.message || 'No se pudo actualizar la venta.' });
    }
  }
);
...
app.delete(
  '/sales/:clientId/:saleId',
  requireAuth,
  requireAnyPermission(['sales:delete', 'inventory:manage', 'read:all']),
  async (req, res) => {
    const { clientId, saleId } = req.params;
    if (req.user.clientId && req.user.clientId !== clientId) {
      return res.status(403).json({ message: 'Sin acceso al cliente.' });
    ...
    try {
      const result = await deleteSale(clientId, saleId);
      return res.json(result);
    } catch (error) {
      return res.status(400).json({ message: error?.message || 'No se pudo eliminar la venta.' });
    }
  }
);
...
app.get('/admin/users/:id/modules', requireAuth, requirePermission('users:manage'), async (req, res) => {
  if (!req.user.roles?.includes('superuser')) {
    return res.status(403).json({ message: 'Solo superuser.' });
  }
  const modules = await listUserModules(req.params.id);
  return res.json(modules);
});

app.put('/admin/users/:id/modules', requireAuth, requirePermission('users:manage'), async (req, res) => {
  if (!req.user.roles?.includes('superuser')) {
    return res.status(403).json({ message: 'Solo superuser.' });
  }
  ...
  await logAudit({
    ...
    details: { userId: req.params.id, modules }
  });
  return res.json({ ok: true });
});
...
app.get('/modules/me', requireAuth, async (req, res) => {
  ...
  const modules = await listUserModules(req.user.sub);
  return res.json(modules.map((m) => ({ key: m.key, enabled: m.enabled })));
});

app.get(
  '/remisiones/:clientId/clients',
  requireAuth,
  requireAnyPermission(['remisiones:view', 'remisiones:manage', 'read:all']),
  async (req, res) => {
    const { clientId } = req.params;
    if (req.user.clientId && req.user.clientId !== clientId) {
      return res.status(403).json({ message: 'Sin acceso al cliente.' });
    }
    const rows = await listRemisionClients(clientId);
    return res.json(rows);
  }
);
...
app.post(
  '/remisiones/:clientId/clients',
  requireAuth,
  requireAnyPermission(['remisiones:create', 'remisiones:manage', 'read:all']),
  async (req, res) => {
    const { clientId } = req.params;
    if (req.user.clientId && req.user.clientId !== clientId) {
      return res.status(403).json({ message: 'Sin acceso al cliente.' });
    ...
    const created = await createRemisionClient(clientId, {
...
app.get(
  ...
  async (req, res) => {
    ...
    const rows = await listRemisiones(clientId);
    return res.json(rows);
  }
);

app.get(
  '/remisiones/:clientId/:id/lines',
  requireAuth,
  requireAnyPermission(['remisiones:view', 'remisiones:manage', 'read:all']),
  async (req, res) => {
    const { clientId, id } = req.params;
    ...
    const rows = await listRemisionLines(id);
    return res.json(rows);
  }
);
...
app.post(
  ...
  async (req, res) => {
    ...
    const { remisionNumber, remisionClientId, recipient, destination, notes, lines } = req.body || {};
    if (!remisionNumber || !Array.isArray(lines) || lines.length === 0) {
      return res.status(400).json({ message: 'Datos incompletos.' });
    }
    const created = await createRemision(clientId, {
      remisionNumber: String(remisionNumber).trim(),
      remisionClientId,
...
app.put(
  '/remisiones/:clientId/:id',
  requireAuth,
  requireAnyPermission(['remisiones:edit', 'remisiones:manage', 'read:all']),
  async (req, res) => {
    const { clientId, id } = req.params;
    if (req.user.clientId && req.user.clientId !== clientId) {
      return res.status(403).json({ message: 'Sin acceso al cliente.' });
    ...
    try {
      const result = await updateRemision(clientId, id, {
        remisionNumber: String(remisionNumber).trim(),
        remisionClientId,
        recipient: String(recipient || '').trim(),
        destination: String(destination || '').trim(),
      ...
      return res.status(400).json({ message: error?.message || 'No se pudo actualizar la remisión.' });
    }
  }
);
...
app.delete(
  '/remisiones/:clientId/:id',
  requireAuth,
  requireAnyPermission(['remisiones:delete', 'remisiones:manage', 'read:all']),
  async (req, res) => {
    const { clientId, id } = req.params;
    if (req.user.clientId && req.user.clientId !== clientId) {
      return res.status(403).json({ message: 'Sin acceso al cliente.' });
    }
    await deleteRemision(clientId, id);
    return res.json({ ok: true });
  }
);
