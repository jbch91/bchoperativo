...
export async function listUserModules(userId) {
  ...
  if (!clientId) {
    ...
    return all.map((m) => ({ ...m, enabled: true }));
  }

  const clientModules = await listClientModules(clientId);
  const { rows: counts } = await query(
    'SELECT COUNT(*)::int AS total FROM user_modules WHERE user_id = $1',
    [userId]
  );
  const hasConfig = (counts[0]?.total ?? 0) > 0;
  if (!hasConfig) {
  ...
  const { rows: enabledRows } = await query(
    'SELECT module_key FROM user_modules WHERE user_id = $1 AND enabled = TRUE',
    [userId]
  );
  const enabledSet = new Set(enabledRows.map((row) => row.module_key));
  return clientModules.map((module) => ({
    ...module,
    enabled: Boolean(module.enabled) && enabledSet.has(module.key)
  }));
}

export async function updateUserModules(userId, moduleKeys) {
  await query('DELETE FROM user_modules WHERE user_id = $1', [userId]);
  for (const key of moduleKeys) {
    await query(
      'INSERT INTO user_modules (user_id, module_key, enabled) VALUES ($1, $2, TRUE) ON CONFLICT DO NOTHING',
      [userId, key]
    );
  }
}
